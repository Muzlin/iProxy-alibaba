...
async function initCopyFiles() {
  try {
    ...
    if (fs.existsSync(versionFile) && fs.readFileSync(versionFile, 'utf-8') === version && !electronIsDev) {
      ...
      fs.moveSync(path.join(IPROXY_FILES_DIR, '/node/modules'), path.join(IPROXY_FILES_DIR, '/node/node_modules'));
...
function createMainWindow() {
  ...
  window.webContents.session.webRequest.onBeforeSendHeaders(filter, (details, callback) => {
    // @ts-ignore
    const base64encodedData = new Buffer(global.WHISTLE_USERNAME + ':' + global.WHISTLE_PASSWORD).toString('base64');
    callback({
      requestHeaders: {
